...
class GoogleGrabber:
    ...
    def getLinks(self):
        try:
            link_name = self.driver.find_elements("tag name",'a')
            links = []
            for l in link_name:
    ...
    def collectLinks(self, img_url):
        ...
        time.sleep(10)
        elems = driver.find_elements("xpath",self.PHOTO_XPATH)[0]
        elems.click()
        time.sleep(1)
        input = driver.find_elements("xpath",'//*[@id="Ycyxxc"]')[0]
        input.clear()
        ...
        pred_error = False
        try:
            pred = driver.find_element("xpath",self.PRED_XPATH)
        ...
        for num in range(2, self.max_pages+1):
            ...
            try:
                page_n = driver.find_element("link text",str(num))
                page_n.click()
    ...
    def collectLinksLocal(self):
        ...
        for p in pathlist:
            ...
            elems.click()
            time.sleep(1)
            elems = driver.find_element("xpath",self.PHOTO_UPLOAD_XPATH)
            
            elems.click()
            time.sleep(1)
            console.subtask("Inserting Path")
            input_box = driver.find_element("xpath",'//*[@id="awyMjb"]')
